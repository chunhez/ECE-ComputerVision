  ...
  case WM_COMMAND:
    switch (LOWORD(wParam))
	  ...
	  case ID_SHOWACTIVECONTOUR_START:
		  if (lflag == 1)
		    _beginthread(activecontourleft, 0, MainWnd);
		  if (rflag==1)
			_beginthread(activecontourright, 0, MainWnd);
		  if (sflag == 1)
		  {
			  _beginthread(activecontourshiftr, 0, MainWnd);
			  _beginthread(activecontourshiftl, 0, MainWnd);
		  }
		  lflag = 0;
		  rflag = 0;
		  sflag = 0;
...
if (Showactivecontour == 1)
{
	...
	for (i = 0;i < N;i++)
	{
		for (j = -1;j < 1;j++)
			for (ii = -1;ii < 1;ii++)
			{
				SetPixel(hDC, fc[i] + ii, fr[i] + j, RGB(0, 0, 255));	/* color the cursor position red */
				//SetPixel(hDC, fc[lfix] + ii, fr[lfix] + j, RGB(0, 255, 255));	/* color the cursor position red */
			}
	...
	for (i = 0;i < NN;i++)
	{
		...
		for (j = -2;j < 2;j++)
			for (ii = -2;ii < 2;ii++)
			{
				SetPixel(hDC, fc2[i] + ii, fr2[i] + j, RGB(0, 255, 0));	/* color the cursor position red */
				//SetPixel(hDC, fc2[rfix] + ii, fr2[rfix] + j, RGB(0, 255, 255));	/* color the cursor position red */
			}
...
void activecontourleft(HWND AnimationWindowHandle)
{	
	FILE	*fpt;
	unsigned char	*sobel;
	double		*test;
	...
	double		sum1, sum2, grad, av_dis, sum_dis, av_color, sum_color, av_grad, sum_grad, min_ex, min_ex2, min_ex3, min_in1, min_in2, max_ex, max_ex2, max_ex3, max_in1, max_in2, min;
	double		*ex, *ex2, *ex3,*in1, *in2, *en;
	double		min_t, max_t;
	...
	ex = (double *)malloc(1000 * sizeof(double));
	ex2 = (double *)malloc(1000 * sizeof(double));
	ex3 = (double *)malloc(1000 * sizeof(double));
	in1 = (double *)malloc(1000 * sizeof(double));
	in2 = (double *)malloc(1000 * sizeof(double));
		...
		for (i = 0;i < N;i++)
		{
			...
			for (r2 = -windowsize/2; r2 <= windowsize/2; r2++)
				for (c2 = -windowsize/2; c2 <= windowsize/2; c2++)
				{
					...
					ex[k] = pow(sobel[(fr[i] + r2)*DISPLAY_COLS + fc[i] + c2], 2);
					ex2[k] = pow(DisplayImage[(fr[i] + r2)*DISPLAY_COLS + fc[i] + c2]-av_color,2);
					ex3[k] = pow(sobel[(fr[i] + r2)*DISPLAY_COLS + fc[i] + c2]-av_grad,2);
					in1[k] = pow(fr[i] + r2 - fr[i + 1], 2) + pow(fc[i] + c2 - fc[i + 1], 2);
					in2[k] = pow(sqrt(pow(fr[i] + r2 - fr[i + 1], 2) + pow(fc[i] + c2 - fc[i + 1], 2)) - av_dis, 2);
					k++;
				}
			...
			{
				ex[ii] = ex[ii] - min_ex;
				ex2[ii] = ex2[ii] - min_ex2;
				ex3[ii] = ex3[ii] - min_ex3;
				in1[ii] = in1[ii] - min_in1;
				in2[ii] = in2[ii] - min_in2;
			}
			...
			{
				if (max_ex3 != 0)
					ex3[ii] = ex3[ii] / max_ex3;
				if (max_ex2 != 0)
					ex2[ii] = ex2[ii] / max_ex2;
				if (max_ex != 0)
					ex[ii] = ex[ii] / max_ex;
				if (max_in1 != 0)
					in1[ii] = in1[ii] / max_in1;
				if (max_in2 != 0)
					in2[ii] = in2[ii] / max_in2;

				en[ii] = -1*ex[ii] + 2*in1[ii] + 4*in2[ii] + 1*ex2[ii] + 1.4*ex3[ii];
			}
			min = en[0];
			countt = 0;
			for (ii = 0; ii < pow(windowsize, 2);ii++)
...
void activecontourright(HWND AnimationWindowHandle)
{
	FILE	*fpt;
	unsigned char	*sobel;
	double		*test;
		...
		for (i = 0;i < NN;i++)
		{
			...
			for (r2 = -windowsize/2; r2 <= windowsize/2; r2++)
				for (c2 = -windowsize/2; c2 <= windowsize/2; c2++)
				{
					...
					ex[k] = pow(sobel[(fr2[i] + r2)*DISPLAY_COLS + fc2[i] + c2], 2);
					ex2[k] = pow(DisplayImage[(fr2[i] + r2)*DISPLAY_COLS + fc2[i] + c2] - av_color, 2);
					ex3[k] = pow(sobel[(fr2[i] + r2)*DISPLAY_COLS + fc2[i] + c2] - av_grad, 2);
					in1[k] = pow(fr2[i] + r2 - fr2[i + 1], 2) + pow(fc2[i] + c2 - fc2[i + 1], 2);
					in2[k] = pow(sqrt(pow(fr2[i] + r2 - fr2[i + 1], 2) + pow(fc2[i] + c2 - fc2[i + 1], 2)) - av_dis, 2);
					k++;
				}
			...
			for (ii = 0; ii < pow(windowsize, 2);ii++)
			{
				...
				if (jj < 40)
					...
					wedge = -1.3;
				*/
				en[ii] = -1*ex[ii] - 2*in1[ii] + 1.5*in2[ii] + 1.2*ex2[ii] + 1*ex3[ii];
			}
			min = en[0];
			countt = 0;
			for (ii = 0; ii < pow(windowsize, 2);ii++)
...
void activecontourshiftl(HWND AnimationWindowHandle)
{
	FILE	*fpt;
	unsigned char	*sobel;
	double		*test;
	...
	double		sum1, sum2, grad, av_dis, sum_dis, min_ex, min_in1, min_in2, max_ex, max_in1, max_in2, min;
	double		*ex, *in1, *in2, *en;
	double		min_t, max_t;
	char	text[320];
	HDC		hDC;
	int		eweight;
	...
	sobel = (unsigned char *)calloc(DISPLAY_COLS*DISPLAY_ROWS, sizeof(unsigned char));
	fcontour = (unsigned char *)calloc(DISPLAY_COLS*DISPLAY_ROWS, sizeof(unsigned char));
	ex = (double *)malloc(500 * sizeof(double));
	in1 = (double *)malloc(500 * sizeof(double));
	in2 = (double *)malloc(500 * sizeof(double));
	...
	for (jj = 0;jj < 80;jj++)
	{
		...
		for (i = 0;i < N;i++)
		{
			...
			for (ii = 0; ii < 49;ii++)
			{
				...
				if (max_in2 != 0)
					in2[ii] = in2[ii] / max_in2;
				en[ii] = -1*ex[ii] + 0*in1[ii] + 2*in2[ii];
			}
			min = en[0];
			countt = 0;
			for (ii = 0; ii < 49;ii++)
...
void activecontourshiftr(HWND AnimationWindowHandle)
{
	FILE	*fpt;
	unsigned char	*sobel;
	double		*test;
